...
bool InplaceBuilder::setExtraBlock(BlockSource& source, BlockPos const& pos, Block const& block) const {
    return source.setExtraBlock(pos, block, updateFlags);
}

bool InplaceBuilder::setBiome(BlockSource& source, BlockPos const& pos, Biome const& biome) const {
