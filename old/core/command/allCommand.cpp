...
void updateFile() {
    ...
    Global<CommandRegistry>->setSoftEnumValues(
        ...
        getWEFiles(
            "models",
            [](std::string_view s) -> bool { return s.substr(s.size() - 4) == ".obj"; }
        )
    );
}
...
void commandsSetup() {
            ...
            addFileWatch(
                ...
                [&](const std::filesystem::path& path,
                    ...
                    case filewatch::Event::added:
                        Global<CommandRegistry>->addSoftEnumValues(
                            "imagefilename",
                            fileEnum
                        );
                        break;
                    ...
                    case filewatch::Event::removed:
                        Global<CommandRegistry>->removeSoftEnumValues(
                            "imagefilename",
                            fileEnum
                        );
                        break;
    ...
    //     [](DynamicCommand const& command, CommandOrigin const& origin, CommandOutput&
    //     output,
    ...
    DynamicCommand::setup(
        ...
        [](DynamicCommand const&                                    command,
           ...
           std::unordered_map<std::string, DynamicCommand::Result>& results) {
            ...
            auto states =
                results["blockstates"].get<std::vector<BlockStateCommandParam>>();

            output.success(BlockTypeRegistry::getBlockNameFromNameHash(
                results["block"].get<CommandBlockName>()
            ));
