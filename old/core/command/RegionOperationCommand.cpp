...
void regionOperationCommandSetup() {
    ...
    DynamicCommand::setup(
        ...
        {{"block", "extraLength", "radius", "args"},
         {"blockPattern", "extraLength", "radius", "args"}},
        // dynamic command callback
        [](DynamicCommand const&                                    command,
           CommandOrigin const&                                     origin,
           CommandOutput&                                           output,
           std::unordered_map<std::string, DynamicCommand::Result>& results) {
            ...
            if (playerData.region != nullptr && playerData.region->hasSelected()) {
                ...
                if (region->regionType != RegionType::CUBOID
                    && region->regionType != CONVEX) {
                ...
                try {
                    ...
                    region->forEachLine([&](BlockPos const& pos1, BlockPos const& pos2) {
                        if (extraLength > 1 && (pos1.x != pos2.x || pos1.z != pos2.z)) {
                            ...
                            double dh =
                                sqrt(pow2(pos1.x - pos2.x) + pow2(pos1.z - pos2.z));
                            ...
                            for (double i = 0; i <= 1; i += df) {
                                ...
                                if (radius == 0) {
                                    ...
                                    BoundingBox(
                                        ...
                                        .forEachBlockInBox([&](BlockPos const& posk) {
                                            if (sqrt(
                    ...
                    if (playerData.maxHistoryLength > 0) {
                        ...
                        boundingBox.forEachBlockInBox([&](BlockPos const& pos) {
                            auto localPos      = pos - boundingBox.min;
                            auto blockInstance = blockSource->getBlockInstance(pos);
                            history.storeBlock(blockInstance, localPos);
                        });
                ...
                try {
                            ...
                            BoundingBox(
                                ...
                                .forEachBlockInBox([&](BlockPos const& posk) {
                                    if ((pos - posk).length() <= 0.5 + radius) {
                                        ...
                                        tmp.at(
                                            (localPos.y + sizeDim.y * localPos.z)
                                                * sizeDim.x
                                            + localPos.x
                                        ) = true;
                                    }
                    ...
                    if (arg_h) {
                        ...
                        boundingBox.forEachBlockInBox([&](BlockPos const& pos) {
                            ...
                            if (tmp[(localPos.y + sizeDim.y * localPos.z) * sizeDim.x
                                    + localPos.x]) {
                                int counts = 0;

                                for (auto& calPos : localPos.getNeighbors()) {
                                    if (tmp[(calPos.y + sizeDim.y * calPos.z) * sizeDim.x
                                            + calPos.x]) {
                                        counts++;
                                    }
                                }
                                if (counts < 6)
                                    tmp2
                                        [(localPos.y + sizeDim.y * localPos.z) * sizeDim.x
                                         + localPos.x] = true;
                            }
                        });
        ...
        [](DynamicCommand const&                                    command,
           ...
           std::unordered_map<std::string, DynamicCommand::Result>& results) {
            ...
            if (playerData.region != nullptr && playerData.region->hasSelected()) {
                ...
                if (region->regionType != RegionType::CONVEX
                    && region->regionType != RegionType::LOFT) {
                ...
                if (region->regionType == RegionType::LOFT) {
                    try {
                        if (playerData.maxHistoryLength > 0) {
                            auto& history = playerData.getNextHistory();
                            history =
                                std::move(Clipboard(boundingBox.max - boundingBox.min));
                            history.playerRelPos.x = dimID;
                            history.playerPos      = boundingBox.min;
                    ...
                    if (arg_r) {
                        for (auto& pos :
                             static_cast<ConvexRegion*>(region.get())->indexedVertices) {
    ...
    DynamicCommand::setup(
        ...
        {ParamData("times", ParamType::Int, true, "times"),
            ...
            if (playerData.region != nullptr && playerData.region->hasSelected()) {
                ...
                if (playerData.maxHistoryLength > 0) {
                    ...
                    *history = Clipboard(boundingBoxHistory.max - boundingBoxHistory.min);
                    ...
                    history =
                        new Clipboard(boundingBoxHistory.max - boundingBoxHistory.min);
                }
                history->playerRelPos.x = dimID;
                history->playerPos      = boundingBoxHistory.min;

                boundingBoxHistory.forEachBlockInBox([&](BlockPos const& pos) {
                ...
                phmap::flat_hash_map<std::string, double> variables;
                playerData.setVarByPlayer(variables);
                region->forEachBlockInRegion([&](BlockPos const& posk) {
                    auto localPos = posk - boundingBoxHistory.min;
                    for (int j = 1; j <= times; ++j) {
                        auto pos = posk + movingVec * j;
                        setFunction(variables, f, boundingBox, playerPos, pos, center);
                        if ((arg_a
                             && (&blockSource->getBlock(posk) != BedrockBlocks::mAir))
                            || !arg_a) {
                            auto& set = history->getSet(localPos);
                            if (set.blocks.has_value())
                                if ((arg_l
                                     && (set.blocks.value().first != BedrockBlocks::mAir))
                                    || !arg_l) {
                                    i += set.setBlock(
                                        pos,
                                        blockSource,
           ...
           std::unordered_map<std::string, DynamicCommand::Result>& results) {
            ...
            if (playerData.region != nullptr && playerData.region->hasSelected()) {
                ...
                if (playerData.maxHistoryLength > 0) {
                    history  = &playerData.getNextHistory();
                    *history = std::move(
                        Clipboard(boundingBoxHistory.max - boundingBoxHistory.min)
                    );
                } else {
                    history =
                        new Clipboard(boundingBoxHistory.max - boundingBoxHistory.min);
                ...
                boundingBoxLast.forEachBlockInBox([&](BlockPos const& pos) {
                    auto localPos = pos - boundingBoxHistory.min;
                ...
                try {
                    ...
                    region->forEachBlockInRegion([&](BlockPos const& pos) {
                        ...
                        if (cpp_eval::eval<double>(genfunc, variables, f) > 0.5) {
                            tmp[(localPos.y + sizeDim.y * localPos.z) * sizeDim.x
                                + localPos.x] = true;
                        }
                        caled
                            [(localPos.y + sizeDim.y * localPos.z) * sizeDim.x
                    ...
                    if (arg_h) {
                        ...
                        region->forEachBlockInRegion([&](BlockPos const& pos) {
                            ...
                            if (tmp[(localPos.y + sizeDim.y * localPos.z) * sizeDim.x
                                ...
                                for (auto& calPos : localPos.getNeighbors()) {
                                    if (!caled
                                            [(calPos.y + sizeDim.y * calPos.z) * sizeDim.x
                                             + calPos.x]) {
                                        setFunction(
                                            variables,
                                            f,
                                            boundingBox,
                                            playerPos,
                                            calPos + boundingBox.min - 1,
                                            center
                                        );
                                        tmp[(calPos.y + sizeDim.y * calPos.z) * sizeDim.x
                                            + calPos.x] =
                                            cpp_eval::eval<double>(genfunc, variables, f)
                                            > 0.5;
           ...
           std::unordered_map<std::string, DynamicCommand::Result>& results) {
            ...
            if (playerData.region != nullptr && playerData.region->hasSelected()) {
                ...
                if (region->regionType == RegionType::LOFT) {
                    ...
                    if (!(loft->circle)) {
                        loft->forEachBlockInLines(2, false, [&](BlockPos const& pos) {
                            setFunction(
                                variables,
                                f,
                                boundingBox,
                    ...
                    region->forEachBlockInRegion([&](BlockPos const& pos) {
                        ...
                        int counts = 0;
                        for (auto& calPos : pos.getNeighbors()) {
                            if (calPos.y <= boundingBox.max.y
                                && calPos.y >= boundingBox.min.y) {
           ...
           std::unordered_map<std::string, DynamicCommand::Result>& results) {
            ...
            if (playerData.region != nullptr && playerData.region->hasSelected()) {
                ...
                if (playerData.maxHistoryLength > 0) {
                    ...
                    region->forTopBlockInRegion([&](BlockPos const& pos) {
                        auto localPos      = pos - boundingBox.min;
                        auto blockInstance = blockSource->getBlockInstance(pos);
                        history.storeBlock(blockInstance, localPos);
                    });
        ...
        [](DynamicCommand const&                                    command,
           ...
           std::unordered_map<std::string, DynamicCommand::Result>& results) {
            ...
            if (playerData.region != nullptr && playerData.region->hasSelected()) {
                ...
                region->forTopBlockInRegion([&](BlockPos const& posk) {
                    BlockPos pos(posk.x, posk.y - 1, posk.z);
                    if (&blockSource->getBlock(pos) == StaticVanillaBlocks::mStone
                        && region->contains(pos)) {
                        setFunction(variables, f, boundingBox, playerPos, pos, center);
        ...
        [](DynamicCommand const&                                    command,
           ...
           std::unordered_map<std::string, DynamicCommand::Result>& results) {
            ...
            if (playerData.region != nullptr && playerData.region->hasSelected()) {
                ...
                std::ofstream outStr(
                    WE_DIR + "structures/" + filename + ".mcstructure",
                ...
                if (arg_l) {
                    ...
                    std::ofstream outStr(
                        WE_DIR + "structures/" + filename + "_list.txt",
                        std::ios::out
                    );
        ...
        [](DynamicCommand const&                                    command,
           ...
           std::unordered_map<std::string, DynamicCommand::Result>& results) {
            ...
            if (playerData.region != nullptr && playerData.region->hasSelected()) {
                ...
                region->forEachBlockUVInRegion([&](BlockPos const& pos, double u, double v
                                               ) {
                    setFunction(variables, f, boundingBox, playerPos, pos, center);
                    ...
                    if (RNG::rand<double>() <= color.a) {
                        i +=
                            playerData
                                .setBlockSimple(blockSource, f, variables, pos, minBlock);
                    }
                });
    ...
    DynamicCommand::setup(
        ...
        {ParamData("item", ParamType::Item, true, "item"),
         ParamData("num", ParamType::Int, true, "num"),
                    ...
                    if (blockInstance.hasContainer()) {
                        ...
                        for (auto& constItem : container->getAllSlots()) {
                            ...
                            if (iNbt->contains("tag", Tag::Type::Compound)
                                ...
                                auto& cmap =
                                    iNbt->getCompound("tag")->getList("Items")->value();
                                std::vector<Tag*> afterVal;
                                afterVal.clear();
                                for (auto& mItem : cmap) {
                                    auto* shulkItem =
                                        ItemStack::create(mItem->asCompoundTag()->clone()
                                        );
                                    ...
                                    if (count2 <= 0
                                        || !(
                                            ...
                                            || shulkItem->sameItemAndAuxAndBlockData(
                                                cmdItem
                                            )
                                        )
                                        || (data >= -2140000000
                                            && data != shulkItem->getAuxValue())) {
                                        delete shulkItem;
                                        continue;
                                    }
                                    int removeNum2 =
                                        num > 0 ? std::min(num, count2) : count2;
                                    i += removeNum2;
                                    if (num > 0) {
                                        num -= removeNum2;
                                    }
