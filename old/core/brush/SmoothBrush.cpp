...
namespace we {
SmoothBrush::SmoothBrush(unsigned short s, int a, float d)
: Brush(s, nullptr),
  ksize(a),
  density(d) {}
long long SmoothBrush::set(Player* player, BlockInstance blockInstance) {
    ...
    auto                range = reinterpret_cast<Dimension*>(
                     Global<Level>->getDimension(dimID).mHandle.lock().get()
    )
                     ->getHeightRange();
    for (int x = box.min.x; x <= box.max.x; ++x)
        for (int z = box.min.z; z <= box.max.z; ++z) {
            auto topy =
                getHighestTerrainBlock(blockSource, x, z, range.min, range.max, mask);
    ...
    for (int x = box.min.x; x <= box.max.x; ++x)
        for (int z = box.min.z; z <= box.max.z; ++z) {
            int cx                             = x - box.min.x;
            int cz                             = z - box.min.z;
            smoothedHeightMap[cx * sizez + cz] = std::lerp(
                heightMap[cx * sizez + cz],
                smoothedHeightMap[cx * sizez + cz],
                static_cast<double>(smoothBrushAlpha(
                    static_cast<float>(sqrt(pow2(x - pos.x) + pow2(z - pos.z))),
                    density,
                    1,
                    size
                ))
