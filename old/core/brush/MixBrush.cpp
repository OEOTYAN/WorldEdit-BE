...
long long MixBrush::set(Player* player, BlockInstance blockInstance) {
    ...
    while (!q.empty()) {
        ...
        BoundingBox(pos1 - 1, pos1 + 1).forEachBlockInBox([&](BlockPos const& tmpPos) {
    ...
    for (auto& pos1 : s) {
        ...
        maskFunc(f, variables, [&]() mutable {
            ...
            if (cmap.contains(block)) {
                mce::Color hereColor =
                    useMixboxLerp ? mixboxLerp(
                        cmap[block],
                        finalColor,
                        smoothBrushAlpha(
                            static_cast<float>(pos1.distanceTo(pos0)),
                            density,
                            opacity,
                            size
                        )
                ...
                iter +=
                    playerData.setBlockSimple(blockSource, f, variables, pos1, minBlock);
            }
        });
    }
    return iter;
