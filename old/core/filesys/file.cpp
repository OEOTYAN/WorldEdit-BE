...
std::vector<std::string> getWEFiles(std::string path) { return getFiles(WE_DIR + path); }
...
getFileWatchSet() {
    static phmap::flat_hash_set<std::unique_ptr<class filewatch::FileWatch<std::wstring>>>
        sets;
    return sets;
}
