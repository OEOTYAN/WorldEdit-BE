...
PlayerData::PlayerData() { historyList.resize(0); }
...
bool PlayerData::changeVicePos(BlockInstance blockInstance, bool output) {
    ...
    if (blockInstance == BlockInstance::Null) {
        if (output) player->trSendText("worldedit.selection.empty.explain.secondary");
        return false;
    }
    if (region == nullptr) {
    ...
    if (region->setVicePos(pos, player->getDimensionId())) {
        if (output)
            player->trSendText(
                "worldedit.selection." + region->getName() + ".explain.secondary",
                pos.toString(),
                region->size()
            );
        vicePos     = pos;
        ...
        if (output) player->trSendText("worldedit.selection.failed.explain.secondary");
    }
    return false;
}
...
bool PlayerData::changeMainPos(BlockInstance blockInstance, bool output) {
    Player* player = Global<Level>->getPlayer(xuid);
    if (blockInstance == BlockInstance::Null) {
        if (output) player->trSendText("worldedit.selection.empty.explain.primary");
        return false;
    }
    if (region == nullptr) {
        region = Region::create(RegionType::CUBOID);
    ...
    if (region->setMainPos(pos, player->getDimensionId())) {
        if (output)
            player->trSendText(
                "worldedit.selection." + region->getName() + ".explain.primary",
                pos.toString(),
                region->size()
            );
        mainPos     = pos;
        ...
        if (output) player->trSendText("worldedit.selection.failed.explain.primary");
    }
    return false;
}
...
bool PlayerData::setBlockWithoutcheckGMask(
...
class Clipboard& PlayerData::getCurrentHistory() { return historyList[historyFlag2]; }
