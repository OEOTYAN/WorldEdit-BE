...
long long EvalFunctions::getIndex(const BlockPos& pos) {
    ...
    if (pos.x < normalSearchBox.min.x || pos.y < normalSearchBox.min.y
        || pos.z < normalSearchBox.min.z || pos.x > normalSearchBox.max.x
        || pos.y > normalSearchBox.max.y || pos.z > normalSearchBox.max.z)
...
double
EvalFunctions::operator()(std::string_view name, const std::vector<double>& params) {
    ...
    case do_hash2("saturate"):
        if (size == 3) return std::min(1.0, std::max(0.0, params[0]));
        break;
    case do_hash2("clamp"):
        if (size == 3) return std::min(params[2], std::max(params[1], params[0]));
        break;
    case do_hash2("lerp"):
        if (size == 3) return params[0] * (1 - params[2]) + params[1] * params[2];
        break;
    case do_hash2("mod"):
        if (size == 2) return posfmod(params[0], params[1]);
        break;
    case do_hash2("abs"):
        if (size == 1) return abs(params[0]);
        break;
    case do_hash2("sin"):
        if (size == 1) return sin(params[0]);
        break;
    case do_hash2("cos"):
        if (size == 1) return cos(params[0]);
        break;
    case do_hash2("sign"):
        if (size == 1) return params[0] == 0.0 ? 0.0 : (params[0] > 0.0 ? 1.0 : -1.0);
        break;
    case do_hash2("log10"):
    case do_hash2("lg"):
        if (size == 1) return log10(params[0]);
        break;
    case do_hash2("ln"):
        if (size == 1) return log(params[0]);
        break;
    case do_hash2("log2"):
        if (size == 1) return log2(params[0]);
        break;
    case do_hash2("round"):
        if (size == 1) return round(params[0]);
        break;
    case do_hash2("floor"):
        if (size == 1) return floor(params[0]);
        break;
    case do_hash2("ceil"):
        if (size == 1) return ceil(params[0]);
        break;
    case do_hash2("exp"):
        if (size == 1) return exp(params[0]);
        break;
    case do_hash2("exp2"):
        if (size == 1) return exp2(params[0]);
        break;
    case do_hash2("sqrt"):
        if (size == 1) return sqrt(params[0]);
        break;
    case do_hash2("tan"):
        if (size == 1) return tan(params[0]);
        break;
    case do_hash2("atan"):
        if (size == 1) return atan(params[0]);
        break;
    case do_hash2("atan2"):
        if (size == 2) return atan2(params[0], params[1]);
        break;
    case do_hash2("asin"):
        if (size == 1) return asin(params[0]);
        break;
    case do_hash2("acos"):
        if (size == 1) return acos(params[0]);
        break;
    case do_hash2("sinh"):
        if (size == 1) return sinh(params[0]);
        break;
    case do_hash2("cosh"):
        if (size == 1) return cosh(params[0]);
        break;
    case do_hash2("tanh"):
        if (size == 1) return tanh(params[0]);
        break;
    case do_hash2("gamma"):
        if (size == 1) return tgamma(params[0]);
        break;
        ...
        if (blockdataInitialized) {
            for (auto& hsa : blockSource->getChunkAt(tmp)->getSpawningAreas()) {
                if (((hsa.aabb.max.x - hsa.aabb.min.x + 1) / 2 + hsa.aabb.min.x == tmp.x)
                    && ((hsa.aabb.max.z - hsa.aabb.min.z + 1) / 2 + hsa.aabb.min.z
                        == tmp.z)
                    && tmp.y >= hsa.aabb.min.y) {
                    if (hsa.type == LevelChunk::HardcodedSpawnAreaType::SWAMP_HUT
                        || hsa.type
                               == LevelChunk::HardcodedSpawnAreaType::PILLAGER_OUTPOST) {
    ...
    case do_hash2("hasplayer"):
        if (blockdataInitialized) {
            ...
            if (size == 1) {
                return nullptr
                    != dimension.fetchAnyInteractablePlayer(
                        here.toVec3(),
    ...
    case do_hash2("noplayer"):
        if (blockdataInitialized) {
            ...
            return playerDistance == FLT_MAX
                || playerDistance >= pow2(static_cast<float>(params[0]));
        }
        return 0;
        break;
    ...
    case do_hash2("issurfacesmooth"):
        if (searchBoxInitialized) {
            ...
            return 1.0
                 - static_cast<double>(
                       getSolidMap(tmp - normalSearchDis, tmp + normalSearchDis)
                   )
                       / static_cast<double>(
                           (normalSearchDis * 2 + 1) * normalSearchDis * 2 + 1
                       );
        }
        return 0;
        break;
    ...
    default:
        if (blockdataInitialized) {
            ...
            if (frontIs(sname, "is_")) {
                ...
                return blockSource->getBlock(tmp).getTypeName().find(sname)
                    != std::string::npos;
            }
            return 0;
        }
