...
long long SimpleBuilder::buildCylinder(
    BlockPos       pos,
    int            dim,
    std::string    xuid,
    Pattern*       pattern,
    ...
    auto range =
        reinterpret_cast<Dimension*>(Global<Level>->getDimension(dim).mHandle.lock().get()
        )
            ->getHeightRange();
    ...
    std::function<
        void(EvalFunctions&, phmap::flat_hash_map<std::string, double> const&, std::function<void()> const&)>
        maskLambda;
    if (mask != "") {
        maskLambda = [&](EvalFunctions&                                   func,
    ...
    if (playerData.maxHistoryLength > 0) {
        ...
        box.forEachBlockInBox([&](BlockPos const& blockPos) {
    ...
    for (auto b : bset)
        for (int y = 0; y < height; y++) {
            ...
            maskLambda(f, variables, [&]() mutable {
                i += pattern->setBlock(variables, f, blockSource, b);
            });
            b.y += 1;
        }
...
long long SimpleBuilder::buildSphere(
    BlockPos       pos,
    int            dim,
    std::string    xuid,
    Pattern*       pattern,
...
long long SimpleBuilder::buildCube(
    BlockPos       pos,
    int            dim,
    std::string    xuid,
    Pattern*       pattern,
    ...
    box.forEachBlockInBox([&](const BlockPos blockPos) {
        if (!hollow
            || (blockPos.x == box.min.x || blockPos.x == box.max.x
                || blockPos.y == box.min.y || blockPos.y == box.max.y
                || blockPos.z == box.min.z || blockPos.z == box.max.z)) {
