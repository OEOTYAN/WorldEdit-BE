...
Block const* RawBlock::getBlock(
    ...
    case 2:
        ...
        if (std::holds_alternative<std::string>(blockIds.first)) {
            blockName = getBlockName(static_cast<int>(round(cpp_eval::eval<double>(
                std::get<std::string>(blockIds.first),
                variables,
                funcs
            ))));
        } else {
        ...
        if (blockIds.second.has_value()) {
            ...
            mData         = std::holds_alternative<int>(dataVal)
                              ? std::get<int>(dataVal)
                              : (static_cast<int>(round(cpp_eval::eval<double>(
                          std::get<std::string>(dataVal),
                          variables,
                          funcs
                      ))));
        }
...
RawBlock* BlockListPattern::getRawBlock(
    const phmap::flat_hash_map<::std::string, double>& variables,
    EvalFunctions&                                     funcs
) {
...
Block const* BlockListPattern::getBlock(
    const phmap::flat_hash_map<::std::string, double>& variables,
    EvalFunctions&                                     funcs
) {
...
bool BlockListPattern::setBlock(
    const phmap::flat_hash_map<::std::string, double>& variables,
    EvalFunctions&                                     funcs,
    ...
    bool  res   = playerData->setBlockSimple(
        blockSource,
        funcs,
        variables,
        pos,
...
BlockListPattern::BlockListPattern(std::string_view str_v, std::string_view xuid)
: Pattern(xuid) {
    ...
    while (i < strSize) {
        if (str[i] == '-' || isdigit(str[i])) {
            ...
            if (str[i] == 'r' && str[i + 1] == 't' && str[i + 2] == '\'') {
                ...
                ++i;
                while (i < strSize
                       && (isalpha(str[i]) || str[i] == '_' || isdigit(str[i])
    ...
    for (int iter = 0; iter < blockNum; iter++) {
        ...
        if (blockIter.find("%num") != std::string::npos) {
            ...
            if (tmpName[0] == 'r' && tmpName[1] == 't' && isdigit(tmpName[2])) {
                rawBlockIter.block =
                    Block::create(static_cast<unsigned int>(std::stoi(tmpName.substr(2)))
                    );
                ++rawPtr;
                continue;
            }
            ...
            if (tmpSNBT[0] == '{' && tmpSNBT[1] == '{') {
                ...
                if (tmpSNBTs.size() > 1) {
                    rawBlockIter.exBlock =
                        Block::create(CompoundTag::fromSNBT(tmpSNBTs[1]).get());
                }
                if (tmpSNBTs.size() > 2) {
        ...
        if (blockIter.find(":num") != std::string::npos) {
            std::get<RawBlock::blockid_t>(rawBlockIter.block).second =
                std::stoi(raw[rawPtr]);
            ++rawPtr;
        } else if (blockIter.find(":funciton") != std::string::npos) {
        ...
        if (std::holds_alternative<RawBlock::blockid_t>(rawBlockIter.block)
            && std::holds_alternative<BlockNameType>(
                std::get<RawBlock::blockid_t>(rawBlockIter.block).first
            )
            && !(
            ...
            auto& blockName = std::get<BlockNameType>(
                                  std::get<RawBlock::blockid_t>(rawBlockIter.block).first
            )
                                  .val;
            if (isBEBlock(blockName)) {
                rawBlockIter.block = Block::create(
                    blockName,
                    std::get<int>(std::get<RawBlock::blockid_t>(rawBlockIter.block)
                                      .second.value_or(0))
                );
